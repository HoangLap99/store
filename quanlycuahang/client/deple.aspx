<%@ Page Title="Dép Lê" Language="C#" MasterPageFile="~/marterpage/client.Master" AutoEventWireup="true" CodeBehind="deple.aspx.cs" Inherits="quanlycuahang.client.deple" %>
     ...
     <div style="margin:40px 50px 50px 50px"> 
         ...
         <asp:DataList ID="DataList1" runat="server" DataKeyField="proID" DataSourceID="SqlDataSource1" RepeatColumns="5" RepeatDirection="Horizontal">
             <ItemTemplate>
               ...
               <br />
                 <h4><asp:Label ID="proNameLabel" runat="server" Text='<%# Eval("proName") %>' />
                 <br /></h4>
                 Mã :
...
<br />
         ...
         <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DataShopConnectionString %>" SelectCommand="SELECT [proID], [proName], [proPrice], [proDiscount], [proPhoto] FROM [Products] WHERE ([typeID] = @typeID)">
             <SelectParameters>
                 <asp:ControlParameter ControlID="DataList1" DefaultValue="8" Name="typeID" PropertyName="SelectedValue" Type="Int32" />
             </SelectParameters>
         </asp:SqlDataSource>
